...
class Bill{
    ...
    {
        $bill_fee = 2;
        $account1 = Accounts::generateInstancedAccountByID($dbc, $bill->getAccount1ID());
        $account2 = Accounts::generateInstancedAccountByID($dbc, $bill->getAccount2ID());
        ...
        if ($balance1 >= $amount + $bill_fee) {
            ...
            transferInstanciation($dbc, $account1, $account2, $amount);

            $instancedBranch = new Branch($dbc, $account1->findBranchID($dbc));
            $instancedBranch->instantiateOwnBankAcc($dbc);
            $instancedBranchAccount1 = $instancedBranch->getBranchAccount();
            transferAccountUpdate($dbc, $account1, $instancedBranchAccount1, 2);
            transferInstanciation($dbc, $account1, $instancedBranchAccount1, 2);
