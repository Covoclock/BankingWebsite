...
class Branch
{
    ...
    {
        $revenue = 0;
        for($i =0; $i<count($this->getAccountIDList()) ;$i++)
        {
    ...
    public function instantiateAccountObjList($conn)
    {
        ...
        {
	    $temp =  Accounts::generateInstancedAccountByID($conn,$AccountIDList[$i]);
		var_dump($temp);
            $this->AccountOBJList[$i] = $temp;
        }
	var_dump($this->AccountOBJList);
